...
function Section({ articles, error, category }) {
  ...
  const articlesList = () => {
    if (articles.length === 0) {
      return <HandelError image="online-articles.svg" text='لا تتوجد  مقالات في هذا القسم الان' />
    } else {
  ...
  return (
    <>
      <header className="section-item" style={{
        ...
        <div className="container h-100 d-flex align-items-center">
          <div className="row">
            <div className="col-md-8">
              ...
              <p className="text fs-p1 text-white lh-md">{category.description}</p>
            </div>
          </div>
        </div>
...
export async function getServerSideProps({ query, params }) {
  ...
  try {
    const { articles } = await fetchApi(`articles/category/?page=${page}&category=${params.name}`);
    return {
      props: {
        articles,
        category,
